...
\cxappendixsection{Additional results for the mixed-norm question of Sah, Sawhney, Stoner and Zhao}
\label{sec:proofdetails}

This appendix collects the positive results that bound where the mixed-norm
counterexamples of \Cref{thm:mixed-general,thm:mixed} can live.  None of them is
...
\]
by \Cref{prop:pone}, and the right-hand side is $\|A^{\mathsf T}A\|_{L_{p,p}}^{1/2}\|B^{\mathsf T}B\|_{L_{q,q}}^{1/2}$.
\end{proof}
...
\]
and \Cref{lem:dualitybound} bounds the first factor by $\|A^*A\|_{L_{p,p}}$ and the second by $\|B^*B\|_{L_{\infty,\infty}}$.
\end{proof}
...
\end{proposition}
\begin{proof}
Write $Z^{\odot p}=X^{\mathsf T}X$.  Then $\|Z\|_{L_{p,q}}=\|X^{\mathsf T}X\|_{L_{1,q/p}}^{1/p}$, and \Cref{prop:pone} bounds this by $\bigl(\|X^{\mathsf T}X\|_{L_{1,1}}\|X^{\mathsf T}X\|_{L_{q/p,q/p}}\bigr)^{1/2p}=\|Z\|_{L_{p,p}}^{1/2}\|Z\|_{L_{q,q}}^{1/2}$.
\end{proof}
...
\end{corollary}
\begin{proof}
For $n\le3$ a positive semidefinite $Z$ has $|Z|:=[|z_{ij}|]$ positive semidefinite as well.\footnote{This is a classical fact about small Hermitian matrices; the source manuscript for this case attributes it to Marcus and Watkins (1969), a reference the present text has not independently checked.}  Since $p\ge1\ge n-2$, the FitzGerald--Horn theorem \citep{FitzGeraldHorn1977} makes $|Z|^{\odot p}$ positive semidefinite, so \Cref{prop:infdiv} applies to $|Z|$.  The mixed norms depend only on $|z_{ij}|$, so the bound transfers to $Z$.
\end{proof}

Together, \Cref{prop:pone,prop:intp,prop:qinf,cor:smalln} confine any counterexample
to non-integer $p$, finite $q$, and order at least four.  \Cref{thm:mixed-general}
lives at $p=q=3/2$ and order four (as a Gram matrix of two $3\times2$ blocks), and
\Cref{thm:mixed} at $(6/5,6)$ and order $21$ --- in each case the smallest region the
...
The construction of \Cref{thm:mixed-general} sidesteps the point by exhibiting a
