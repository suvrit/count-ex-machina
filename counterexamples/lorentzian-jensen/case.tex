...
\cxtitle{Lorentzian Jensen--Bregman metric principle}
...
S.~Sra, standalone problem; formulation as recorded in \S1 of the case's \path{paper.tex}
\end{cxsource}
...
Strictly Lorentzian rational cubic and three rational points; exact rational triangle-violation bound $>7.15\times10^{-3}$.
\end{cxcertificate}

% ---------------- result: log-volume-distance ----------------

\begin{cxsource}{log-volume-distance}
S.~Sra, standalone problem (geometric specialization of the same principle); formulation as recorded in \S1 of the case's \path{paper.tex}
\end{cxsource}
...
Shephard realization of the same cubic as a Steiner polynomial in $\R^3$; the violation transfers verbatim.
\end{cxcertificate}
