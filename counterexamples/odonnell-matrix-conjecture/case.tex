...
$D=\operatorname{Diag}(R)=\operatorname{diag}(d_1,\ldots,d_n)$ be its diagonal
...
\[
  \epsilon:=\lVert D-\Lambda(R)\rVert_1=\sum_i|\lambda_i-d_{ii}|
\]
measures how far the diagonal sits from the spectrum.  The source states its
conclusion asymptotically, as $\lVert R-D\rVert_1=O(\sqrt\epsilon)$; an
...
O'Donnell in personal communication, a fact the public answer itself records.
\end{cxsource}
...
general, and then conjectures the normalised case:
