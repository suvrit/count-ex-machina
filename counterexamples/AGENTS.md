...
counterexamples/<case-id>/
  case.json      the single source of truth — everything else is generated from it
  context.tex    our own setup prose: notation the quoted statement needs
  statement.tex  the statement as originally posed  (statement-<result-id>.tex if several)
  dossier.tex    the paper body: conjecture, theorem, proof
  ...
  paper.tex      standalone write-up — withheld cases only, instead of a dossier
```
...
- `dossier` — filename, must exist.
- `context` — filename of the setup prose (rendered before the quoted
  statements, outside the quote boxes, so our words are never mistaken for the
  source's).
- `bib_keys` — every key must exist in `tex/references.bib`.
- `results[]` — one entry per refuted statement:
  - `id` — globally unique across all cases; this is what humans cite.
  ...
  - `statement_tex`, `certificate_tex` — the two ledger columns, one line each.
  ...
  - `theorem_label` — must appear verbatim as `\label{...}` in the dossier.
  - `provenance` — `source_tex` (who posed it and where, with `\cite{...}`),
    `url` and `retrieved` (the copy consulted; `url` may be a repo-relative
    path when the statement was posed in a file here), `fidelity`
    (`verbatim` **only** if transcribed from the source's own text, else
    `paraphrase`), and `statement` (the sidecar filename).
- `credits` — `posed_by`, `found_by`, `formalized_by`, `audited_by`,
  `contributed_by`. `found_by` is `{"model", "date"}` or a **list** of them
  (`date` matches `YYYY-MM` or `YYYY-MM-DD`). Credits may be overridden
  per result and merge key by key over the case block, so a result differing
  only in `found_by` inherits the rest. Unknown keys are an error.
- `verify` — `python` (entry script), `sage` (list of cross-checks),
  `requires` (extra pip deps).
- `artifacts[]` — `file` + `description`; a TODO description fails the build.

## Prose lives in `.tex`, never in JSON

`case.json` names a file; `tools/build.py` inlines it. Write
`$\mathcal{Y}$`, not `"$\\mathcal{Y}$"` on one collapsed line. Putting the
prose back into the JSON (`context_tex`, `provenance.statement_tex`) is a build
error, not a silent fallback. Whole-line `%` comments in a sidecar are stripped
before the paper sees them — which is also why leaving a template comment in
place cannot hide an unfilled TODO.

Two results must not name the same statement file: that would attribute one
source's words to the other statement.

## dossier.tex house style

Body only. The `\section` / `\subsection` heading and the credit line are
**generated** — writing them yourself duplicates them in the PDF.
...
The dossier is `\input` from `tex/`, so any relative path inside it resolves
against `tex/`, and the paper compiles only from that directory. Preamble
macros available to you (do not redefine them) are listed in `tex/AGENTS.md`.
...
| `sidecar file 'x.tex' does not exist` | create it, or fix the filename in `case.json` |
| `'context_tex' is no longer read from case.json` | move the prose into `context.tex`, name it in `context` |
| `provenance 'statement_tex' is no longer read from case.json` | move the quote into `statement.tex`, name it in `provenance.statement` |
| `statement file 'x' is already used by result Y` | one file per result: `statement-<result-id>.tex` |
...
| `\label{thm:x} not found in dossier.tex` | make `theorem_label` and the dossier `\label` identical |
| `bib key 'k' not found` | add the entry to `tex/references.bib` |
| `unbalanced braces in ...` | count `{}` in that field or sidecar |
| `credits.found_by needs a real model and date` | supply the real model and `YYYY-MM`, or leave it and report it — **never guess** |
| `provenance needs source_tex, and the statement as originally posed` | same: find the source or leave the TODO standing |
...
```sh
make regen        # today, while the twelve found_by TODOs stand:
                  #   .venv/bin/python tools/build.py --allow-todo
make check        # no error may name your case; the pre-existing 14 may remain
